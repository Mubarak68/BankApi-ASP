...
namespace WebApplication1.Controllers
{
    ...
    public class BankController : ControllerBase
    {
            ...
            [ProducesResponseType(StatusCodes.Status500InternalServerError)]
            public IActionResult AddBranch(AddBankBranch request)
            {
                try
                {
                    ...
                    return Created(nameof(Details), new { Id = branch.BankId });
                }
                catch (Exception ex)
                {
                    return this.StatusCode(StatusCodes.Status500InternalServerError, ex);
            ...
            public ActionResult<BankBranchResponse> Details(int id)
            {
                try
                {
                    var bank = _context.BankBranches.Where(b => b.BankId == id).Include(b => b.Employees).SingleOrDefault();
                    if (bank == null)
                    {
                        return NotFound();
                    }
                    return new BankBranchResponse
                    {
                        ...
                        employeeResponses = bank.Employees.Select(employee => new EmployeeResponse { Name = employee.Name, CivilId = employee.CivilId, Position = employee.Position })
                    };
                }
                catch (Exception ex)
                {
            ...
            [ProducesResponseType(StatusCodes.Status404NotFound)]
            [ProducesResponseType(StatusCodes.Status500InternalServerError)]
            public IActionResult Edit(int id, AddBankBranch request)
            {
                try
                {
